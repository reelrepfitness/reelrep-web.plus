  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({ name, email, phone: phone || undefined, turnstileToken }),
      })
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-md mx-auto space-y-5" dir="rtl">
      ...
      <Button
        ...
        className="w-full bg-teal-500 hover:bg-teal-600 text-white font-bold text-base h-12 disabled:opacity-40"
      >
        {status === "loading" ? (
